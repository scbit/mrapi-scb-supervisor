...
async getStableDailyReview(scope,conversationId){const id=`stable__${Buffer.from(String(scope||'guide_v1')).toString('base64url').slice(0,80)}__${Buffer.from(String(conversationId)).toString('base64url').slice(0,160)}`;const d=await this.db.collection(this.c.dailyReviews).doc(id).get();return d.exists?d.data():null}
async saveStableDailyReview(scope,conversationId,data){const id=`stable__${Buffer.from(String(scope||'guide_v1')).toString('base64url').slice(0,80)}__${Buffer.from(String(conversationId)).toString('base64url').slice(0,160)}`;await this.db.collection(this.c.dailyReviews).doc(id).set({scope:String(scope||'guide_v1'),conversationId,...data,updatedAt:new Date().toISOString()},{merge:true})}
...
async listLiveDailyReports(limit=200,date=null){
  const s=await this.db.collection(this.c.liveDailyReports).limit(Math.min(Number(limit||200),1000)).get();
  return s.docs.map(d=>({id:d.id,...d.data()}))
    .filter(x=>!date||String(x.date||'')===String(date))
    .sort((a,b)=>String(b.generatedAt||'').localeCompare(String(a.generatedAt||'')));
}
async saveCriticalIncident(id,data){
  await this.db.collection(this.c.criticalIncidents).doc(String(id)).set({...data,id:String(id),updatedAt:new Date().toISOString()},{merge:true});
}
async listCriticalIncidents(limit=200,status=null){
  const s=await this.db.collection(this.c.criticalIncidents).limit(Math.min(Number(limit||200),1000)).get();
  return s.docs.map(d=>({id:d.id,...d.data()}))
    .filter(x=>!status||String(x.status||'')===String(status))
    .sort((a,b)=>String(b.detectedAt||b.updatedAt||'').localeCompare(String(a.detectedAt||a.updatedAt||'')));
}
