...
function detectMessageActor(r={}){const d=normalizeDirection(r.direction||r.dir||r.type);const j=[r.direction,r.type,r.source,r.provider,r.user,r.userEmail,r.owner,r.agent,r.sentBy,r.profileName,r.createdBy,r.author,r.role].map(v=>String(v||'').toLowerCase()).join(' ');if(d==='inbound')return'client';const human=!!String(r.user||r.userEmail||r.owner||r.agent||r.sentBy||r.profileName||r.createdBy||r.author||'').trim();if(d==='outbound'&&human&&!j.includes('bot')&&!j.includes('auto'))return'human';if(j.includes('bot')||j.includes('automat')||j.includes('system'))return'bot';return d==='outbound'?'outbound_unknown':'unknown'}
