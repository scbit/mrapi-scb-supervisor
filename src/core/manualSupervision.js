...
function rowCase(row){const p=product(row);return{conversationId:row.conversationId,hubConversationId:row.hubConversationId||row.conversationId,contactName:row.contactName||'Sin nombre',seller:row.seller||row.owner||'',product:p,profile:profile(row),severity:severity(row),needsCorrection:needsCorrection(row),reason:caseReason(row),expected:expected(row),hubUrl:row.hubUrl||hubUrl(row.hubConversationId||row.conversationId),lastClientText:row.lastClientText||'',lastHumanText:row.lastHumanText||'',lateMinutes:row.maxHumanResponseMinutes||0,responseStatus:responseStatus(row),followUpStatus:followUpLabel(row),followUpAttempts:Number(row.followUpAttemptsAfterLastClient||0),checklist:checklist(row),opportunity:p.defined===true,status:displayStatus(row),evidenceSignature:evidenceSignature(row)}}
