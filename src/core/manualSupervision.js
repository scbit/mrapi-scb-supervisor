...
function manualReportKey(date,cutoff=17,sellerKey=null){const seller=sellerKey?`__seller_${Buffer.from(norm(sellerKey)).toString('base64url').slice(0,80)}`:'';return `${date}__cutoff_${Number(cutoff)}__guide_v1_concrete_advance_v1${seller}`}
class ManualSupervisionService{
  ...
  async base(date,cutoff=17,forceAi=false,sellerKey=null){
    ...
    return this.dailyService.generate({date,endHour:Number(cutoff),forceAi,reportKey,reviewScope:'guide_v1_concrete_advance_v1',sellerKey})
  }
