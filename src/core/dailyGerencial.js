...
function buildLeadQualityInsights(deals,rows){
  ...
  for(const d of excellent){
    ...
    const item={dealId:d.id,leadQuality:'EXCELENTE',seller:d.owner||row.seller||'No detectado',contactName:row.contactName||d.title||'sin dato',conversationId:row.conversationId||d.conversationId||'',hubUrl:row.hubUrl||hubUrl(row.hubConversationId||row.conversationId||d.conversationId),reasons,score:row.ai?.overall_score??null,status:status(row)};
...
function analyzeConversation(conversation,messages,range,lateMinutes=30){
  ...
  return{
    conversationId:conversation.id,hubConversationId:conversation.hubConversationId||conversation.id,contactName:conversation.contactName||conversation.phone||'sin dato',phone:conversation.phone||'',owner:conversation.owner||seller,seller,stage:conversation.stage||'',dealId:conversation.dealId||'',sourceChannel:conversation.sourceChannel||'',
