...
function createApp({engine,dailyService,databases,config,telegramAdapter,emailAdapter,bucketAdapter}){
  ...
  async function sendDailyTelegram(report){const sent=await telegram.send(report.text);const at=new Date().toISOString();await engine.store.saveDailyReport(report.date,{...report,telegramSent:true,telegramMessageId:sent.messageId,telegramSentAt:at});await saveComm('telegram',sent);return sent}
