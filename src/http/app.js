...
function createApp({engine,dailyService,databases,config,telegramAdapter,emailAdapter,bucketAdapter}){
  ...
  app.post(['/api/supervisor/daily/start','/api/supervisor/daily/process','/api/supervisor/daily/status','/api/supervisor/daily/generate'],auth,async(q,r)=>{try{if(!dailyService)throw new Error('DAILY_SERVICE_NOT_AVAILABLE');const report=await dailyService.generate({date:q.body?.date||undefined,now:q.body?.now?new Date(q.body.now):new Date()});r.json({ok:true,report})}catch(e){r.status(500).json({ok:false,error:e.message})}});
