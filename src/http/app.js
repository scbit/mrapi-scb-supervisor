...
function createApp({engine,dailyService,dailyV3LiveService,manualSupervisionService,remoteService,databases,config,telegramAdapter,emailAdapter,bucketAdapter}){
  ...
  function manualGuideReportKey(date,cutoff=17,sellerKey=null){const seller=sellerKey?`__seller_${Buffer.from(String(sellerKey).trim().toLowerCase()).toString('base64url').slice(0,80)}`:'';return `${date}__cutoff_${Number(cutoff)}__guide_v1_product_context_v1${seller}`}
  app.post('/api/supervisor/manual/start',auth,async(q,r)=>{try{
    ...
    const result=await dailyService.start({date,startHour:9,endHour:cutoff,lateMinutes:30,limit:500,forceAi:false,reportKey,reviewScope:'guide_v1_product_context_v1',sellerKey});
