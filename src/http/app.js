...
function createApp({engine,dailyService,dailyV3LiveService,manualSupervisionService,remoteService,databases,config,telegramAdapter,emailAdapter,bucketAdapter}){
  ...
  app.get('/api/integrations/telegram/chats',auth,async(q,r)=>{try{const ids=String(q.query?.ids||'').split(',').map(x=>x.trim()).filter(Boolean).slice(0,100);r.json({ok:true,chats:await telegram.listRecentChats(ids)})}catch(e){r.status(500).json({ok:false,error:e.message})}});
  ...
  function manualGuideReportKey(date,cutoff=17,sellerKey=null){const seller=sellerKey?`__seller_${Buffer.from(String(sellerKey).trim().toLowerCase()).toString('base64url').slice(0,80)}`:'';return `${date}__cutoff_${Number(cutoff)}__guide_v1_product_context_v2_fresh${seller}`}
  app.post('/api/supervisor/manual/start',auth,async(q,r)=>{try{
    ...
    const result=await dailyService.start({date,startHour:9,endHour:cutoff,lateMinutes:30,limit:500,forceAi:q.body?.refresh===true,reportKey,reviewScope:'guide_v1_product_context_v2_fresh',sellerKey});
  ...
  async function superSupervisorChatId(){const setup=await remoteService.getNetworkSetup();return setup.settings?.weekday?.superSupervisorChatId||null}
