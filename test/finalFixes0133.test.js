...
test('0.13.4 live report hides SUPERSEDED and keeps product without discovery attribution',()=>{
  ...
  assert.ok(!t.includes('SUPERSEDED'));assert.ok(t.includes('PENDIENTE'));assert.ok(t.includes('Producto: Máquina'));assert.ok(!t.includes('Producto descubierto por'));
});
